...
export const MyResumesSection = ({ refreshKey }: { refreshKey?: number }) => {
  ...
  return (
    <div className="mt-2 border-t border-border px-4 pt-4">
      <Link to="/resumes"
        className="label-eyebrow mb-2 flex items-center justify-between px-3 transition-smooth hover:text-foreground">
        {t("nav.resumes")}
      ...
      {loading ? (
        ...
        <Link to="/resumes"
          className="block rounded-xl px-3 py-2.5 text-sm text-muted-foreground transition-smooth hover:bg-secondary hover:text-foreground">
          {t("resumes.empty")}
        </Link>
      ) : (
        <div className="space-y-1">
          {resumes.slice(0, 6).map((r) => (
            <Link key={r.id} to="/resumes"
              title={r.name}
